...
class LoaderSTDGAT(Dataset):
    def __init__(self, args, mode='train'):
        ...
        self.feature = np.load(self.args.od_feature, allow_pickle=True)
        ...
        if self.mode == 'train':
            ...
            self.feature = self.feature[:-(30+31)*24]
        elif self.mode == 'val':
            # 9
            self.data = self.data[-(30+31)*24:-31*24]
            self.feature = self.feature[-(30+31)*24:-31*24]
        elif self.mode == 'test':
            # 10
            self.data = self.data[-(31*24+self.seq_len):]
            self.feature = self.feature[-(31*24+self.seq_len):]
        else:
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.seq_len+1):
            ...
            feature_list.append(self.feature[index + i])
        return sample, feature_list[:-1]
